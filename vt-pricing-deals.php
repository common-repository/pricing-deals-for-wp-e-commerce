<?php
/*
Plugin Name: VarkTech Pricing Deals for WP E-Commerce
Plugin URI: http://varktech.com
Description: An e-commerce add-on for WP-E-Commerce, supplying Pricing Deals functionality.
Version: 1.0.1.1      
Author: Vark
...
*/
   ...
   error_reporting(E_ERROR | E_CORE_ERROR | E_COMPILE_ERROR); //v1.0.1.1
        
class VTPRD_Controller{
	
	public function __construct(){    

		define('VTPRD_VERSION',                               '1.0.1.1');
    define('VTPRD_MINIMUM_PRO_VERSION',                   '1.0.1.1');
    ...
    define('VTPRD_PRO_PLUGIN_NAME',                      'Varktech Pricing Deals Pro for WP E-Commerce');    //v1.0.2
   
    require_once ( VTPRD_DIRNAME . '/wpsc-integration/vtprd-parent-definitions.php');
 ...
 *************************************************** */
    ...
    require_once  ( VTPRD_DIRNAME . '/wpsc-integration/vtprd-parent-functions.php');
    require_once  ( VTPRD_DIRNAME . '/wpsc-integration/vtprd-parent-theme-functions.php');
    require_once  ( VTPRD_DIRNAME . '/wpsc-integration/vtprd-parent-cart-validation.php');   
    require_once  ( VTPRD_DIRNAME . '/wpsc-integration/vtprd-parent-definitions.php');
    ...
    if(defined('VTPRD_PRO_DIRNAME')) {
      ...
      require_once  ( VTPRD_PRO_DIRNAME . '/wpsc-integration/vtprd-lifetime-functions.php' );          
    } else {
  ...
  function vtprd_enqueue_frontend_scripts(){
    global $vtprd_setup_options;
        
    wp_enqueue_script('jquery'); //needed universally
    
    //ajax actions - add-to-cart, clear cart, etc...
    //actual JS
    wp_register_script('vtprd-discount-script', VTPRD_URL.'/wpsc-integration/js/vtprd-discount-script-min.js' );  
   
    //create ajax resources
    wp_localize_script('vtprd-discount-script', 'empty_cart_discount', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )  ));        
    wp_localize_script('vtprd-discount-script', 'add_to_cart_discount', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )  ));
    wp_localize_script('vtprd-discount-script', 'get_nanosecond_delay', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )  ));             
     
    wp_enqueue_script('vtprd-discount-script');
   ...
   if (version_compare(PHP_VERSION, VTPRD_EARLIEST_ALLOWED_PHP_VERSION) < 0) {    //'<0' = 1st value is lower  
    ...
    if(defined('WPSC_VERSION') && (VTPRD_PARENT_PLUGIN_NAME == 'WP E-Commerce') ) { 
      ...
      if( (version_compare(strval($new_version), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower 
        $message  =  '<strong>' . __('Looks like you\'re running an older version of WP E-Commerce.   You need to be running at least ** WP E-Commerce 3.8 **, to use the Varktech Pricing Deals plugin. ' , 'vtprd') . '</strong>' ;
        $message .=  '<br>' . __('Your current WP E-Commerce version = ' , 'vtprd') .WPSC_VERSION;
        $admin_notices = '<div id="message" class="error fade" style="background-color: #FFEBE8 !important;"><p>' . $message . ' </p></div>';
        add_action( 'admin_notices', create_function( '', "echo '$admin_notices';" ) );
    ...
    if (VTPRD_PARENT_PLUGIN_NAME == 'WP E-Commerce') {
        $message  =  '<strong>' . __('Varktech Pricing Deals for WP E-Commerce requires that WP E-Commerce be installed and activated. ' , 'vtprd') . '</strong>' ;
        $message .=  '<br>' . __('It looks like WP E-Commerce is either not installed, or not activated. ' , 'vtprd');
        $admin_notices = '<div id="message" class="error fade" style="background-color: #FFEBE8 !important;"><p>' . $message . ' </p></div>';
        add_action( 'admin_notices', create_function( '', "echo '$admin_notices';" ) );
    ...
    if(defined('JIGOSHOP_VERSION') && (VTPRD_PARENT_PLUGIN_NAME == 'JigoShop')) { 
      $new_version =      VTPRD_EARLIEST_ALLOWED_PARENT_VERSION;
      $current_version =  JIGOSHOP_VERSION;
      if( (version_compare(strval($new_version), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower
        $message  =  '<strong>' . __('Looks like you\'re running an older version of JigoShop. You need to be running at least ** JigoShop 1.0 **, to use the Varktech Pricing Deals plugin' , 'vtprd') . '</strong>' ;
        $message .=  '<br>' . __('Your current JigoShop version = ' , 'vtprd') .JIGOSHOP_VERSION;
        $admin_notices = '<div id="message" class="error fade" style="background-color: #FFEBE8 !important;"><p>' . $message . ' </p></div>';
        add_action( 'admin_notices', create_function( '', "echo '$admin_notices';" ) );
    ...
    if (VTPRD_PARENT_PLUGIN_NAME == 'JigoShop') {
        $message  =  '<strong>' . __('Varktech Pricing Deals for JigoShop requires that JigoShop be installed and activated. ' , 'vtprd') . '</strong>' ;
        $message .=  '<br>' . __('It looks like JigoShop is either not installed, or not activated. ' , 'vtprd');
        $admin_notices = '<div id="message" class="error fade" style="background-color: #FFEBE8 !important;"><p>' . $message . ' </p></div>';
        add_action( 'admin_notices', create_function( '', "echo '$admin_notices';" ) );
  ...
  *************************************************** */ 
  ...
  //  $is_this_purchLog = $wpdb->get_var("SHOW TABLES LIKE `".VTPRD_PURCHASE_LOG."` ");
    ...
    if ( $is_this_purchLog  == VTPRD_PURCHASE_LOG) {
      //This column added in version 0.3
      //  table already defined, check if the new column is there.  If not, add it.

      $column_exists = $wpdb->get_results("SHOW COLUMNS FROM `" . VTPRD_PURCHASE_LOG . "` LIKE 'cart_object' ");     
      if (empty($column_exists)) {
         $add_fields = $wpdb->query($wpdb->prepare("ALTER TABLE ".VTPRD_PURCHASE_LOG." ADD cart_object text"));     
      }
      return;
    }
