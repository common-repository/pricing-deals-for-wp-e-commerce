...
function vtprd_setup_options_cntl() {
  ...
  <?php } ?>        
    ...
    <span class="system-info">
       ...
       <?php if(defined('WPSC_VERSION')        && (VTPRD_PARENT_PLUGIN_NAME == 'WP E-Commerce') ) { ?>
 ...
 */ 
  ...
  //****************************
   ...
   //If WPEC version  <  3.8.9 , show the nanosecond switch
//  if( defined('WPSC_VERSION')  && (VTPRD_PARENT_PLUGIN_NAME == 'WP E-Commerce') &&                           Commented v1.0.2
//    ( version_compare(strval('3.8.9'), strval(WPSC_VERSION), '>') == 1) ) {   //'==1' = 2nd value is lower 
    	add_settings_field(	           //opt46
    		'nanosecond_delay_for_add_to_cart_processing',						// ID used to identify the field throughout the theme
    		__( 'Processing Delay for Add to Cart Discount Processing', 'vtprd' ),		// The label to the left of the option interface element        
    		array(&$this, 'vtprd_nanosecond_delay_for_add_to_cart_processing_callback'), // The name of the function responsible for rendering the option interface
    		'vtprd_setup_options_page',	// The page on which this option will be displayed
    		'processing_settings_section',			// The name of the section to which this field belongs
    		array(								// The array of arguments to pass to the callback. In this case, just a description.
    			 __( 'Processing Delay for Add to Cart Discount Processing', 'vtprd' )
    		)
    	);
 // }      v1.02
...
function vtprd_nanosecond_delay_for_add_to_cart_processing_callback() {    //opt46
   //If WPEC version  <  3.8.9 , show the nanosecond switch
  if( defined('WPSC_VERSION')  && (VTPRD_PARENT_PLUGIN_NAME == 'WP E-Commerce') &&
    ( version_compare(strval('3.8.9'), strval(WPSC_VERSION), '>') == 1) ) {   //'==1' = 2nd value is lower 
    	$options = get_option( 'vtprd_setup_options' );	
      $html = '<input type="text" class="mediumText" id="nanosecond_delay_for_add_to_cart_processing"  name="vtprd_setup_options[nanosecond_delay_for_add_to_cart_processing]" value="' . $options['nanosecond_delay_for_add_to_cart_processing'] . '">';
     
      $html .= '&nbsp;&nbsp;<span>'   . __('Nanoseconds ["1000" = one second]', 'vtprd') .  '</span>';
      
      $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id="help46" class="help-anchor" href="javascript:void(0);" >'   . __('SEE => More Info', 'vtprd') .  '</a>';
      
      $html .= '<p id="help46-text" class = "help-text" >'; 
      $html .= __('Add to Cart discount processing follows the parent add to cart processing, but MUST wait for the parent to be finished, hence the delay.', 'vtprd') 
            .'<br><br>'.
            __('Default value = "1000" (one second).', 'vtprd'); 
      $html .= '</p>';  
...
//test  
error_log( print_r(  'Input at topy of validate', true ) ); //test
error_log( var_export($input, true ) );         //test
  ...
  switch( true ) { 
    ...
    default:   //standard update button hit...                 
      	$output = get_option( 'vtprd_setup_options' );   //v1.0.2 FIX
     ...
     //min nano delay value test.  Only applies if  WPSC_VERSION < '3.8.9', otherwise the value is the default of 1000 
     if ( ( isset( $input['nanosecond_delay_for_add_to_cart_processing'] ) ) &&          //v1.0.2
          ($input['nanosecond_delay_for_add_to_cart_processing'] < 250 ) )  {
        $admin_errorMsg = __(' Nanosecond Delay must be at least 1/4 of a second or a value of "250".  Minimum value of "500" recommended, default is "1000".', 'vtprd');
        $admin_errorMsgTitle = __('Processing Delay for Add to Cart Discount Processing', 'vtprd');
