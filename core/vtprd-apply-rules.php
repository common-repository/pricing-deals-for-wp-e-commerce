...
class VTPRD_Apply_Rules{
	...
	public function __construct(){
		global $wpsc_cart, $vtprd_cart, $vtprd_rules_set, $vtprd_info, $vtprd_setup_options, $vtprd_rule;
    ...
    if ($vtprd_info['current_processing_request'] == 'cart') {  
      ...
      */
      vtprd_compute_wpsc_coupon_discount();
      //**********************
     ...
     //Move parent cart contents to vtprd_cart 
      ...
      //the pre_process_cart_for_autoAdds rolls out any previous auto adds, so do it again... 
      vtprd_compute_wpsc_coupon_discount();             
  ...
  public function vtprd_process_actionPop_and_discount($i, $d, $br, $ar ) {      
    ...
    */
    switch( true ) {
      ...
      case   ($vtprd_rules_set[$i]->rule_deal_info[$d]['discount_amt_type']   == 'percent'):  //percent as each/all is mathematically the same, easier to do it as each.
 ...
 */ 
  ...
  //***********************************************************     
   ...
   //**********************************
   public function vtprd_clear_temp_storage_checkout_coupon_id() { 
     global $vtprd_info;
